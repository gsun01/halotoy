...
def get_all_grb_data(output_base_dir):
    ...
    for run_dir in sorted(output_base_dir.glob("GRB_*")):
        ...
        data = np.genfromtxt(
            ...
            usecols=("E","theta_obs","phi_obs")
        )
        if data.size == 0:
            continue

        E     = np.concatenate((E, data["E"].ravel()))
        theta = np.concatenate((theta, data["theta_obs"].ravel()))
        phi   = np.concatenate((phi, data["phi_obs"].ravel()))
    
    E = 0.32 * (E/20)**2  # observed E in GeV
    return E, theta, phi
...
def main():
    ...
    # define E bin edges: 10 log bins from E_min to E_max
    ...
    for i in range(n_bins):
        ...
        I_sm = hp.smoothing(I, fwhm=beam, verbose=False)
        ...
        fits_out = output_base_dir / f"Healpix_map_{E_lo:.2e}-{E_hi:.2e}GeV_NSIDE{nside}.fits"
        hp.write_map(fits_out, I, overwrite=True)
        print(f"Saved Healpix map fits to {fits_out}")

        txt_out = output_base_dir / f"Cl_{E_lo:.2e}GeV_NSIDE{nside}.txt"
        ...
        hp.mollview(
            logI_sm,
            title=f"[{E_lo:.2e}, {E_hi:.2e}] GeV (NSIDE={nside})",
            unit=r"Log(Intensity [$cm^{-2} s^{-1} sr^{-1}$])",
            norm="hist",
            badcolor="black",
        )
        svg_out = output_base_dir / f"Healpix_map_{E_lo:.2e}GeV_NSIDE{nside}.svg"
        ...
        plt.title(f"Auto-APS [{E_lo:.2e}, {E_hi:.2e}] GeV")
        # plt.legend()
        plt.tight_layout()
        svg_out = output_base_dir / f"Auto_APS_{E_lo:.2e}GeV_NSIDE{nside}.svg"
        plt.savefig(svg_out, dpi=300)
        plt.close()
